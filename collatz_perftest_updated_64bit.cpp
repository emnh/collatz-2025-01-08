...
constexpr uint64_t MAX_ITERATIONS = 1024;
constexpr uint64_t K = 1 << 30;
constexpr uint64_t CACHE_SIZE = K;
constexpr int UNINITIALIZED = -1;
//constexpr uint64_t BASE_BITS = 20;
// constexpr unsigned int S_BITS = 30;

std::string uint128_to_string(uint64_t value) {
...
std::vector<uint64_t> generate_S(const uint64_t BASE_BITS) {
    std::vector<uint64_t> S;
    const uint64_t max_nL = (uint64_t) 1 << BASE_BITS;

    for (uint64_t nL = 0; nL < max_nL; ++nL) {
        uint64_t b = (uint64_t) 1 << BASE_BITS; // Start with b = 2^d
        uint64_t c = nL;
...
std::vector<uint64_t> initialize_powers_of_3() {
    std::vector<uint64_t> powers(129, 1);
...
inline int count_trailing_zeros_128(const uint64_t n) {
    if (n == 0) return 128;
...
inline std::pair<uint64_t, int> truncate_and_count_zeroes(uint64_t n) {
...
int convergence_test_iterative(uint64_t n, const std::vector<uint64_t>& powers_of_3, int* cache, std::pair<uint64_t, int>* intermediate) {
    int delay = 0;
...
int convergence_test_recursive(const uint64_t n0_input, const uint64_t n_input, const std::vector<uint64_t>& powers_of_3, int* cache, int depth = 0) {
    if (n_input < n0_input) {
        return 0;
    }
    ...
    const uint64_t n_incremented = n_input + 1;
    const auto [truncated_n, a] = truncate_and_count_zeroes(n_incremented);
    const uint64_t n_multiplied = truncated_n * powers_of_3[a] - 1;
...
int main2(uint64_t BASE_BITS = 20) {
    ...
    std::pair<uint64_t, int> intermediate[MAX_ITERATIONS];
    ...
    // const uint64_t unskipped_iterations = (K >> BASE_BITS) * (1 >> BASE_BITS);
    const uint64_t iterations = (K >> BASE_BITS) * S.size();
    for (unsigned int mH = 1; mH <= K >> BASE_BITS; ++mH) {
        for (unsigned int i = 0; i < S.size(); ++i) {
            const uint64_t mL = S[i];
            const uint64_t n = (static_cast<uint64_t>(mH) << BASE_BITS) + mL;
